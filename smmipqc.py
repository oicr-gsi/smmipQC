...
def set_up_analysis(args):
    ...
    newfile = open(bashscript, 'w')
    mycmd = 'module load smmipqc; sleep 60; smmipqc link -fpr {0} -r {1} -pr {2} -w {3}'.format(args.fpr, args.run, args.project, args.workingdir) 
    newfile.write(mycmd)
    newfile.close()
    ...
    newfile = open(bashscript, 'w')
    newfile.write('module load smmipqc; sleep 60; smmipqc qsubs -r {0} -pr {1} -w {2} -p {3}'.format(args.run, args.project, args.workingdir, args.panel)) 
    newfile.close()
    ...
    newfile = open(bashscript, 'w')
    newfile.write('module load smmipqc; sleep 60; smmipqc qc -r {0} -pr {1} -w {2} -m {3}'.format(args.run, args.project, args.workingdir, args.max_jobs)) 
    newfile.close()
    ...
    newfile = open(bashscript, 'w')
    newfile.write('module load smmipqc; sleep 60; smmipqc merge -r {0} -pr {1} -w {2}'.format(args.run, args.project, args.workingdir)) 
    newfile.close()
